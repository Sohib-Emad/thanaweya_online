...
export 'admin_teachers_password_ext.dart';

class AdminTeachersRepo {
  final SupabaseClient _client;

  AdminTeachersRepo({SupabaseClient? client})
  ...
  Future<ApiResult<List<Map<String, dynamic>>>> getPendingTeachers() async {
    try {
      final data = await _client.from('teachers').select('''
            ...
            users!inner(id, full_name, email, phone, plain_password), subjects(id, name_ar)
      ...
      try {
        final data = await _client.from('teachers').select('''
              ...
              users!inner(id, full_name, email, phone), subjects(id, name_ar)
            ''').eq('approval_status', 'pending').order('created_at', ascending: false);
        return ApiResult.success(List<Map<String, dynamic>>.from(data));
      } catch (err) {
  ...
  Future<ApiResult<void>> approveTeacher(String teacherId) async {
    try {
      await _client.from('teachers').update({'approval_status': 'approved'}).eq('id', teacherId);
  ...
  Future<ApiResult<void>> rejectTeacher(String teacherId, String reason) async {
    try {
      await _client.from('teachers').update({'approval_status': 'rejected', 'rejection_reason': reason}).eq('id', teacherId);
      return const ApiResult.success(null);
    } catch (e) {
      return ApiErrorHandler.handleException(e);
    }
  ...
  Future<ApiResult<void>> toggleRenewalAlert(String teacherId, bool requiresRenewal) async {
    try {
      await _client.from('teachers').update({'requires_renewal': requiresRenewal}).eq('id', teacherId);
      return const ApiResult.success(null);
    } catch (e) {
      return ApiErrorHandler.handleException(e);
    }
  ...
  Future<ApiResult<void>> toggleBanTeacher(String teacherId, bool isBanned, {String? reason}) async {
    try {
